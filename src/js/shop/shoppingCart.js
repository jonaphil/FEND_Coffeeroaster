...
function getProductCartSpecs(productId, variantId) {
  ...
  const variantDetails = productDetails.variants.find(
    (variantObj) => variantObj.id === variantId
  );
...
function loadShoppingCartFromLocalStorage() {
  ...
  if (shoppingCart) {
    shoppingCart.forEach((item) =>
      createShoppingCartEntry(item.productId, item.variantId, item.amount)
    );
...
function saveCartToLocalStorage() {
  const localStorageList = shoppingCartArray.map(
    ({ id, productId, variantId, amount }) => ({
      id,
      productId,
      variantId,
      amount,
...
function createShoppingCartProductNode(cartProductObj) {
  ...
  const removeByButton = () => {
    removeFromShoppingCart(
      getCartIndex(cartProductObj.productId, cartProductObj.variantId)
    );
  };
...
function createShoppingCartWrapperEl() {
  ...
  if (oldShoppingCart) {
    console.log(
      "Error generating shopping Cart. Shopping Cart already exists."
    );
    return -1;
  }
...
function updateShoppingCartWrapperEl() {
  shoppingCartEl.querySelector(
    ".shopping-cart__heading p"
  ).innerHTML = `${getShoppingCartAmountTotal()} Produkte`;
}
...
function createShoppingCartEntry(productId, variantId, amount) {
  ...
  const shoppingCartItemDiv =
    createShoppingCartProductNode(shoppingCartItemObj);
...
function updateShoppingCartEntry(cartIndex, amount) {
  ...
  cartEntryNodes.querySelector(
    ".shopping-cart__product__specs__details"
  ).innerHTML = `${amount} Stück<br>${shoppingCartArray[cartIndex].variantName}`;
...
export function removeFromShoppingCart(indexInCart, amount = null) {
  const newAmount =
    amount === null ? 0 : shoppingCartArray[indexInCart].amount - amount;
...
export function getShoppingCartAmountTotal() {
  const totalAmount =
    shoppingCartArray.length > 0
      ? shoppingCartArray.reduce(
          (preSum, cartProduct) => preSum + cartProduct.amount,
          0
        )
      : 0;
  shoppingCartArray.length > 0
...
export function getShoppingCartPriceTotal(shippingCost = 0) {
  const totalPrice = shoppingCartArray.reduce(
    (preSum, cartProduct) => preSum + cartProduct.variant.price,
    shippingCost
  );
  return totalPrice;
}
