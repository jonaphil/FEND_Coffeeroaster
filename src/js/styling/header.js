import {
  toggleShoppingCart,
  getShoppingCartAmountTotal,
  shoppingCartEl,
} from "/js/shop/shoppingCart.js";
...
const headerShoppingCart = headerElement.querySelectorAll(
  ".header__shopping-cart"
);
const headerShoppingCartImg = Array.from(headerShoppingCart).map((div) =>
...
function headerShoppingCartImgSetProp(
  propsToChange = {
    src: undefined,
  ...
  if (propsToChange.toggleClass) {
    imgList.forEach((imgObj) =>
      imgObj.classList.toggle(propsToChange.toggleClass)
    );
  }
  if (propsToChange.onClick) {
    imgList.forEach((imgObj) =>
      imgObj.addEventListener("click", propsToChange.onClick)
    );
  }
}
...
function changeHeaderToWhite() {
  ...
  headerShoppingCartImgSetProp({
    src: "/images/icons/Shopping-bag-black.svg",
  });
}
...
export function updateShoppingCartPatch() {
  ...
  const updateSinglePatch = (patchDiv) => {
    if (amount > 0) {
      patchDiv.classList.contains("hidden")
